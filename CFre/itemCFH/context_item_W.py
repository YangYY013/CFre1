...
if __name__ == '__main__':
    t1=time.time()
    contextTemp = readFile("D:/PycharmProjects/untitled2/itemCFH/Context_9000train.csv")  # 读取用户-物品-上下文
    X=1
    Y=1-X
    context_dic = createDictContext(contextTemp)  # 创建上下文字典
    Wc = contextCF(context_dic,Y)
    print("创建物品上下文相似矩阵")

    item_dic = createDictItem(contextTemp)  # 创建物品字典
    Wn = itemCF(item_dic,X)
    print("创建物品用户相似矩阵")

    #计算 W 物品相似度矩阵
    Wn = readWcn("D:/PycharmProjects/untitled2/itemCFH/W_n_n.txt")
    Wc = readWcn("D:/PycharmProjects/untitled2/itemCFH/W_c_c.txt")
